...
chcp 65001 > nul
echo ========================================
echo UVキャッシュクリーンアップタスクをタスクスケジューラーに登録します...
echo ========================================
echo.
...
REM XMLファイルの存在確認
if not exist "UvCacheCleanTask.xml" (
    echo エラー: UvCacheCleanTask.xml が見つかりません。
    echo 現在のディレクトリ: %CD%
    echo ファイル一覧:
    dir /b
    pause
...
echo UvCacheCleanTask.xml が見つかりました。
echo.
...
REM タスクをインポート
echo 新しいタスクを登録中...
schtasks /create /xml "UvCacheCleanTask.xml" /tn "UV Cache Clean Task"
...
if %errorlevel% equ 0 (
    ...
    echo.
    echo タスクの詳細を確認中...
    schtasks /query /tn "UV Cache Clean Task" /fo list
) else (
    echo.
    echo ========================================
    ...
    echo schtasks /create /xml "UvCacheCleanTask.xml" /tn "UV Cache Clean Task" /v
)
