...
3. ログファイルでエラー内容を確認
4. **ログオン状態**: このタスクは「ユーザーがログオンしているときのみ実行」する設定（InteractiveToken）です。スケジュール時刻（毎週月曜10時）に**ユーザーがログオンしていない**と、タスクは実行されません。ログオン中に手動で「実行」すれば動作確認できます。

#### ログオンしていなくても実行したい場合

「ユーザーがログオンしているかどうかに関わらず実行する」に変更する手順：

1. タスクスケジューラで「Chocolatey Update Task」を右クリック → 「プロパティ」
2. 「全般」タブで「ユーザーがログオンしているかどうかにかかわらず実行する」を選択
3. 「OK」をクリックし、表示されたユーザーアカウントのパスワードを入力してタスクを更新する

または、タスク登録後に `schtasks /change /tn "Chocolatey Update Task" /ru "%USERNAME%" /rp` でパスワードを設定する方法もあります（`register_choco_update_task.bat` の XML インポートだけではパスワードは設定されません）。
